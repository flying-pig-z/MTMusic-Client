...
class MyLikeView extends StatefulWidget {
  const MyLikeView({super.key});

  @override
  State<MyLikeView> createState() => _MyLikeViewState();
}

class _MyLikeViewState extends State<MyLikeView> {
  int MyWorkCount = 0;
  List MyWorkNames = [];
  List Songid = [];
  List coverPath = [];
...
//从这里开始是我要修改的请求接口
  ...
  Future<void> _fetchMyLikesData() async {
    try {
      MyLikes bean2 = await SonglistApi().getMyLike(
        ...
        body: Container(
          ...
          child: Column(
            children: [
              ...
              Expanded(
                child: MyWorkCount == 0
                    ? Center(child: Text('你还没有点赞')) // 如果没有作品，显示提示文本
                    : ListView.builder(
